...
def check_folder(p: Path, name: str):
    ...
    if not classes:
        raise FileNotFoundError(f"{name} ichida class papkalar yo‘q.")
    return classes


def make_class_weights_from_targets(targets, num_classes):
...
def make_weighted_sampler(targets, num_classes):
    class_weights, counts = make_class_weights_from_targets(targets, num_classes)
...
def per_class_report_from_confusion(cm, class_names):
    ...
    for i, cls in enumerate(class_names):
        ...
        rows.append({
            ...
            "accuracy_within_class": round(recall, 4),
        })
...
def main():
    ...
    ap.add_argument("--test_dir", required=True, help="Test dataset papkasi")
    ...
    sampler, class_weights, class_counts = make_weighted_sampler(
        train_targets,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Eng muhim joy: VeryMild va NonDemented ni kuzatish
        if "NonDemented" in train_ds.classes and "VeryMildDemented" in train_ds.classes:
            nd_idx = train_ds.class_to_idx["NonDemented"]
            vm_idx = train_ds.class_to_idx["VeryMildDemented"]
            print(
                f"[ND->VM confusion] True NonDemented predicted VeryMildDemented: {cm[nd_idx, vm_idx].item()} | "
                f"True VeryMildDemented predicted NonDemented: {cm[vm_idx, nd_idx].item()}"
            )
